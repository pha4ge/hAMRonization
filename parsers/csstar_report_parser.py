...
#blaSHV* blaSHV-100*     NODE_11_length_157792_cov_39.2115       95.111% 900     900
FIELD_MAP = {
    0: "gene_symbol",
    1: "gene_name",
    3: "contig_id",
    ...
    with open(path_to_report) as report_file:
        ...
        for row in reader:
            for key in clean_fields:
                row[key] = row[key].replace('*', '').replace('?', '').replace('TR$', '').replace('$', '')
...
def prepare_for_amr_class(parsed_report, additional_fields={}):
    ...
    for field, amr_result_field in FIELD_MAP.items():
        if amr_result_field:
            input_for_amr_class[str(amr_result_field)] = parsed_report[field]
