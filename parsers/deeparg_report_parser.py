...
#ARG    query-start     query-end       read_id predicted_ARG-class     best-hit        probability     identity        alignment-lengthalignment-bitscore      alignment-evalue        counts
FIELD_MAP = {
    "#ARG": 'gene_symbol',
...
def parse_report(path_to_report):
    ...
    """
    report_fieldnames = [x for x in FIELD_MAP.keys() if x not in ['_reference_accession']]
    ...
    with open(path_to_report) as report_file:
        ...
        float_fields = []
        for row in reader:
            # reference_accession is combined into gene_name
            row['_reference_accession'] = row['best-hit'].split('|')[0]
...
def main(args):
    ...
    additional_fields['reference_database_id'] = "DeepARG"
