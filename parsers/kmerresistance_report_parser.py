...
MANDATORY_FIELDS = {'input_file_name',
                    ...
                    'sequence_identity',
                    #'reference_database_id',
                    #excluded as gene_name/gene_symbol extraction only works for resfinder
                    'reference_database_version',
                    'reference_accession',
                    #'analysis_software_name',
...
def parse_report(path_to_report):
    ...
    """
    report_fieldnames = [x for x in FIELD_MAP.keys() if x not in ['_gene_name',
                                                                  '_gene_symbol']]
    ...
    with open(path_to_report) as report_file:
        ...
        float_fields = []
        for row in reader:
            row['_gene_name'] = "_".join(row['#Template'].split('_')[:-1])
            row['_gene_symbol'] = row['#Template'].split('_')[0]
